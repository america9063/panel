...
echo 'bouquet="' . ($setting->user_agent ? $setting->user_agent : 'HLSSERVER') . '";';
echo 'directory="/etc/enigma2/iptv.sh";';
echo 'url="http://' . $setting->webip . ':' . $setting->webport  . '/plist/' .$username  . '/' .$password  . '/e2";';
...
echo 'if !cat /etc/enigma2/bouquets.tv | grep -v grep | grep -c $bouquet > /dev/null;then echo "[+]Creating Folder for hlsserver.com iptv and rehashing...";';
...
header('Content-Disposition: attachment; filename="hlsserverEnigmaFile.sh"');
